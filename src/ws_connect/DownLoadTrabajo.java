...
public class DownLoadTrabajo extends AsyncTask<String, Integer, Integer>{ //doInBakGround, Progress, onPostExecute
	...
	public DownLoadTrabajo(Context context, String Directorio){
		...
		this.FcnInSolicitudes			= new ClassInSolicitudes(this.ConnectServerContext, this.DirectorioConexionServer);
	...
	protected Integer doInBackground(String... params) {
		...
		try{
			...
			if(response.toString()==null) {
				...
				try {
					this.FcnArch.ByteArrayToFile(resultado, "Trabajo.txt");
					this.InformacionDescarga = FcnArch.FileToArrayString("Trabajo.txt",false);
					this.FcnInSolicitudes.InsertSolicitudes(this.InformacionDescarga);
					this.FcnArch.DeleteFile("Trabajo.txt");
					_retorno = 1;
				} catch (IOException e) {
					e.printStackTrace();
					_retorno = -3;
